...
int map[CONSOLE_WIDTH+2][CONSOLE_HEIGHT+2];
// 0: 공백
// 1: block
// 2: Out of Map
// 3+.. -> Another
...
int main()
{
    ...
    while( 1 ){
        if( isPlaying ){
            ...
            if( ((int)rand()%1000) < ITEM_RANDOM_DROP ){// 무작위 확률로 아이템 드랍
                if( item_count < ITEM_MAX_COUNT ){
                    switch( ((int)(rand()))%10 ){
                    case 0:case 1:case 2:case 3:
                        //BLOCK_ITEM_LINE
                        RX = rand()%CONSOLE_WIDTH;
                        RY = rand()%(CONSOLE_HEIGHT-(PLAYER_USER_MIN_Y+1)) + PLAYER_USER_MIN_Y+1;
                        if( RY < CONSOLE_HEIGHT && RY > PLAYER_USER_MIN_Y){
